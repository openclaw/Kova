...
function evaluateCase(testCase, observation, injectResult) {
  ...
  return [
    ...
    invariant(`${testCase.id}:reply-target`, expects.replyTo !== "inbound-message" || firstFinal?.replyToId === observation?.inboundEvent?.id, `${testCase.id} did not preserve the inbound reply target`),
    invariant(`${testCase.id}:no-reply-target`, expects.replyTo !== "none" || firstFinal?.replyToId == null, `${testCase.id} unexpectedly attached a reply target`),
